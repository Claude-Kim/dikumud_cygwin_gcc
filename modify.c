...
void check_reboot(void)
{
	...
	extern int shutdown_num, reboot_num;
	...
	if ((t_info->tm_hour + 1) == REBOOT_AT && t_info->tm_min > 30)
		if (boot = fopen("./reboot", "r"))
		{
			if (t_info->tm_min > 50)
			{
				...
				send_to_all("Automatic reboot_num. Come back in a little while.\n\r");
				shutdown_num = reboot_num = 1;
			}
			else if (t_info->tm_min > 40)
...
void gr(int s)
{
	...
	extern int slow_death, shutdown_num;
	...
	if (((ld = load()) >= 6) || (txt = nogames()) || slow_death)
	{
		...
		shutdown_num = 1;				/* this shouldn't happen */
	else if (wnr)
	{
